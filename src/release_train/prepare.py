...
class PreparePlanItem:
    ...
    def gh_commands(self) -> list[str]:
        ...
        cmds = [
            ...
            "# ensure milestone + labels (also done by prepare --apply when gh works)",
            recipe_ensure_milestone(self.repo_full, self.milestone),
            recipe_ensure_label(
                self.repo_full, LABEL_PINS, LABEL_PINS_COLOR, LABEL_PINS_DESCRIPTION
            ),
            recipe_ensure_label(
                self.repo_full,
                LABEL_COMPAT,
...
def plan_prepare(
    ...
    *,
    ...
    Pin PRs are assigned to milestone ``release-train/{minor}`` with label
    ``release-train/pins``. On ``--apply``, milestone + labels are ensured via
    ...
    for member in manifest.plugin_members:
        ...
        if do_ensure:
            try:
                ensure_log = ensure_milestone_and_labels(member.full_name, ms_title, apply=True)
                ensure_applied = True
            ...
            ensure_log = [
                f"# would ensure milestone {ms_title!r} + labels on {member.full_name}",
                recipe_ensure_milestone(member.full_name, ms_title),
                recipe_ensure_label(
                    member.full_name, LABEL_PINS, LABEL_PINS_COLOR, LABEL_PINS_DESCRIPTION
                ),
                recipe_ensure_label(
                    member.full_name,
                    LABEL_COMPAT,
...
def format_prepare_report(result: PrepareResult) -> str:
    ...
    lines.append("")
    lines.append(
        f"GitHub state: milestone `{result.milestone}` + label `{LABEL_PINS}` "
        "(no local state files)."
    )
    lines.append(
        f"Plan: assign pin PRs to milestone `{result.milestone}` with label `{LABEL_PINS}`."
    )
    lines.append("")
    ...
    if result.apply:
        ...
        lines.append(
            "Milestone + label ensure runs via gh when available; "
            "remote PR creation is recipe-based."
        )
        lines.append("`gh` available: " + ("yes" if gh_available() else "no"))
        if result.ensure_applied:
            lines.append("Ensure steps: attempted via gh api / gh label.")
        lines.append("")
        ...
        lines.append("Re-run with --apply to ensure milestones/labels and print gh recipes.")
